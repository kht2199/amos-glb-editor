...
| `Task` | import / validate / apply / export 같은 내부 편집 작업 | partial | 작업 흐름은 있으나 canonical `Task` 타입으로 구조화되어 있지 않음 |
| `ToolCall` | 직접 대응 구조 없음 | missing | 외부 tool orchestration 저장소가 아님 |
| `Policy` | validation rule, export 전 오류 검사 | partial | 승인/권한보다는 편집 규칙/검증에 가깝다 |
...
| Top view editing | 상단뷰 기반 XY 편집 | `src/components/TopViewCanvas.tsx`, `src/App.tsx` | `TopViewCanvas` | aligned | README 원칙과 UI 구조가 일치 |
...
| Validation drawer | 오류/경고 확인 | `src/components/ValidationDrawer.tsx`, `src/lib/validation.ts`, `src/lib/collision.ts` | validation / collision issues | aligned | ID/관계/충돌 검증 구조 존재 |
...
| Task-like editor flow | import / validate / apply / export 흐름 | `src/store/editor-store.ts`, `src/lib/validation.ts`, `src/lib/glb.ts` | load / validate / apply / export | partial | canonical `Task`의 직접 타입은 없고 로컬 편집 절차로만 존재 |
...
| Scenario / ontology-linked constraints | domain ontology와 직접 연결된 제약 엔진 | `src/lib/validation.ts` 일부 | validation rules | partial | 현재는 편집기 로컬 규칙 중심, domain canonical과의 직접 맵은 약함 |

## 현재 코드 기준 핵심 연결
### 1. 앱 조립 레이어
- `src/App.tsx`
- `Toolbar`, `StructurePanel`, `TopViewCanvas`, `InspectorPanel`, `PreviewPanel`, `ValidationDrawer`, `StatusBar`를 묶는다.
- glb-editor의 실제 화면 구성과 사용자 편집 흐름이 여기서 드러난다.

### 2. 상태/편집 레이어
- `src/store/editor-store.ts`
- draft/applied split, selection, mode, duplicate, apply/revert, undo/redo, validation, export orchestration이 모여 있다.
...
### 5. 검증/보조 로직 레이어
- `src/lib/validation.ts`, `src/lib/collision.ts`, `src/lib/utils.ts`
- 충돌 탐지, ID/관계 검증, port 위치 계산, nearest lift 연결, snap 보정 등 편집기 규칙이 분산되어 있다.
...
3. validation은 있지만 canonical `Policy` / `ExecutionTrace` 수준의 구조적 판단 계층은 없다.
...
4. validation rule ↔ domain constraint 매핑 문서 추가
