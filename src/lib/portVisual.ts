...
const materials = {
  ...
  portFrameTop: new THREE.MeshStandardMaterial({ color: '#67e8f9', emissive: '#155e75', emissiveIntensity: 0.95, roughness: 0.32, metalness: 0.14 }),
  portFrameBottom: new THREE.MeshStandardMaterial({ color: '#f59e0b', emissive: '#92400e', emissiveIntensity: 0.95, roughness: 0.34, metalness: 0.14 }),
  portDockTop: new THREE.MeshStandardMaterial({ color: '#0ea5e9', emissive: '#0c4a6e', emissiveIntensity: 0.45, roughness: 0.3, metalness: 0.12 }),
  portDockBottom: new THREE.MeshStandardMaterial({ color: '#fb923c', emissive: '#7c2d12', emissiveIntensity: 0.45, roughness: 0.32, metalness: 0.1 }),
...
function buildPortVisual(group: THREE.Group, port: Pick<PortEntity, 'width' | 'depth' | 'height' | 'semanticRole' | 'portType' | 'level'>) {
  ...
  const isTopLevel = port.level === 'TOP'
  const frameMaterial = isTopLevel ? materials.portFrameTop : materials.portFrameBottom
  const dockMaterial = isTopLevel ? materials.portDockTop : materials.portDockBottom
  ...
  if (isTopLevel) {
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.18, 1.1, 1.1), frameMaterial, -frameWidth * 0.18, frameDepth * 0.26, baseHeight + dockHeight + 0.55)
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.18, 1.1, 1.1), frameMaterial, frameWidth * 0.18, frameDepth * 0.26, baseHeight + dockHeight + 0.55)
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.48, 0.9, 0.8), frameMaterial, 0, frameDepth * 0.42, baseHeight + dockHeight + 1.15)
  } else {
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.48, 0.9, 0.8), frameMaterial, 0, frameDepth * 0.16, baseHeight + dockHeight + 0.2)
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.18, 1.1, 1.1), frameMaterial, -frameWidth * 0.18, frameDepth * 0.02, baseHeight + dockHeight - 0.35)
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.18, 1.1, 1.1), frameMaterial, frameWidth * 0.18, frameDepth * 0.02, baseHeight + dockHeight - 0.35)
  }
}
