...
const materials = {
  portFrame: new THREE.MeshStandardMaterial({ color: '#d6d3d1', roughness: 0.52, metalness: 0.12 }),
  portDock: new THREE.MeshStandardMaterial({ color: '#fb923c', roughness: 0.35, metalness: 0.08 }),
...
function buildPortVisual(group: THREE.Group, port: Pick<PortEntity, 'width' | 'depth' | 'height' | 'semanticRole' | 'portType'>) {
  ...
  addBox(group, new THREE.BoxGeometry(frameWidth, frameDepth, baseHeight), materials.portFrame, 0, 0, baseHeight / 2)
  ...
  addBox(group, new THREE.BoxGeometry(frameWidth * 0.82, dockDepth, dockHeight), materials.portDock, 0, -frameDepth * 0.04, baseHeight + dockHeight / 2)
  ...
  addBox(group, new THREE.BoxGeometry(frameWidth * 0.82, 0.7, 0.5), materials.portFrame, 0, frameDepth * 0.36, baseHeight + dockHeight * 0.7)
  ...
  if (port.semanticRole === 'STOCKER_ACCESS') {
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.94, 1.2, 1.4), materials.portFrame, 0, -frameDepth * 0.52, baseHeight + 0.7)
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.62, 1, 0.9), materials.portAccess, 0, -frameDepth * 0.6, baseHeight + 1.7)
  } else {
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.22, frameDepth * 0.72, 1.1), materials.portFrame, -frameWidth * 0.34, 0, baseHeight + 0.55)
    addBox(group, new THREE.BoxGeometry(frameWidth * 0.08, frameDepth * 0.72, 1.7), materials.portDock, frameWidth * 0.36, 0, baseHeight + 0.85)
  }
}
