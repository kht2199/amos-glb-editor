...
- 편집기 동작, import/export, validation, metadata mapping 문서는 기본적으로 `glb-editor` 저장소 범위 안에서 유지한다.
...
- domain은 semantic reference를 제공하고, `glb-editor` 저장소는 scene editing/object metadata/validation ownership을 가진다.
...
- 이 저장소에서는 `Request`, `Task`, `Policy`, `ExecutionTrace`를 runtime 엔진처럼 억지로 구현하려 하지 말고, 편집 세션 액션 / validation / history / metadata constraint와의 대응 범위를 명확히 적는다.
